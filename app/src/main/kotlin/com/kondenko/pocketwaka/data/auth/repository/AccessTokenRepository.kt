...
import com.kondenko.pocketwaka.utils.editBlock
import io.reactivex.Single
import javax.inject.Inject

@PerApp
class AccessTokenRepository @Inject constructor(val service: AuthService, private val prefs: SharedPreferences) {
    ...
    fun getEncryptedToken(): Single<AccessToken> {
        if (!isTokenSaved()) return Single.error(IllegalStateException("Access Token is not acquired yet"))
        ...
        return Single.just(encryptedToken)
    }
...
//    fun getTokenHeaderValue() = Const.HEADER_BEARER_VALUE_PREFIX + " " + getEncryptedAccessTOken()
    ...
    fun isTokenSaved() = prefs.contains(KEY_ACCESS_TOKEN)
