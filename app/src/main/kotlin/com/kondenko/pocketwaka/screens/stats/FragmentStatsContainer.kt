...
class FragmentStatsContainer : Fragment(), Refreshable {
        ...
        with(stats_smarttablayout_ranges) {
            ...
            setOnTabClickListener {
                if (stats_viewpager_content.currentItem == it) {
                    (pagerAdapter.getPage(it) as? FragmentStats)?.scrollToTop()
                }
            }
    ...
    private fun onFragmentSelected(position: Int) =
          (pagerAdapter.getPage(position) as? FragmentStats?)?.let { fragment ->
              screenTracker.log(activity, Screen.Stats(fragment.arguments?.getString(FragmentStats.argRange)))
    ...
    fun getSelectedTab() =
          (pagerAdapter.getPage(stats_viewpager_content.currentItem) as? FragmentStats?)?.range
    ...
    private fun FragmentPagerItems.Creator.addFragment(@StringRes title: Int, range: String) =
          add(title, FragmentStats::class.java, bundleOf(FragmentStats.argRange to range))
